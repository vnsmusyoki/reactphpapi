...
switch ($method) {
    case 'POST':
        ...
        if (empty($name) || empty($capacity) || empty($status) || empty($closing) || empty($opening)) {
            ...
            if ($queryrows >= 1) {
                ...
                $add = "INSERT INTO `pools`(`pool_name`,`opening_time`,`closing_time`, `capacity`, `status`) VALUES ('$name', '$opening','$closing','$capacity', '$status')";
    ...
    case 'PUT':
        ...
        if (empty($name) || empty($capacity) || empty($status) || empty($closing) || empty($opening)) {
            ...
            $checkuser = "UPDATE  `pools` SET `pool_name`='$name', `capacity`='$capacity',`status`='$status', `opening_time`='$opening', `closing_time`='$closing' WHERE `id`= '$path[6]'";
