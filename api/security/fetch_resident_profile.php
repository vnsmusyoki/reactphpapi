...
switch ($method) {
    ...
    case 'GET':
        ...
        case 'PUT':
            ...
            if (empty($email) || empty($id_number) || empty($name)|| empty($phonenumber)) {
                ...
                $checkuser = "UPDATE  `users` SET `full_names`='$name', `id_number`='$id_number',`phone_number`='$phonenumber', `email`='$email'  WHERE `id`= '$path[6]'";
            ...
            case 'DELETE':
                ...
                if ($queryusers) {
                    $response = ['status' => '1', 'message' => 'Resident Account deleted successfully.'];
                    exit();
                } else {
